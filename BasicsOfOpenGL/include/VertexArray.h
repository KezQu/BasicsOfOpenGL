...
public:
	...
	const unsigned int GetIBSize(const unsigned int index) const;
	const int GetActiveBufferID() const;

	void Bind() const;
	void Unbind() const;
	
	void AddBuffer(const IndexBuffer* ib, const VertexBuffer* vb);
	void SwapBuffers(const unsigned int index) const;
